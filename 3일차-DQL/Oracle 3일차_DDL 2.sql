...
SELECT * FROM EMPLOYEE -- 전체 컬럼은 *로 퉁
-- WHERE EMP_ID = '200';
...
WHERE ROUND(SYSDATE - HIRE_DATE) BETWEEN 1825 AND 3650;
-- 문제2.
-- 재직중이 아닌 직원의 이름,부서코드, 고용일, 근무기간, 퇴직일을 검색하여라 
--(퇴사 여부 : ENT_YN)
SELECT EMP_NAME "이름", DEPT_CODE "부서코드", HIRE_DATE "고용일"
, ROUND(ENT_DATE - HIRE_DATE) "근무기간", ENT_DATE "퇴직일" FROM EMPLOYEE
WHERE ENT_YN = 'Y';
-- 문제3.
...
WHERE FLOOR((SYSDATE - HIRE_DATE)/365) >= 10                      -- CEIL은 올림 FLOOR은 내림 ROUND는 반올림
--ORDER BY "근속년수" ASC;
ORDER BY 3 ASC; -- 컬럼의 순서에 따라서 숫자로 정렬 가능
-- 문제4.
-- 입사일이 99/01/01 ~ 10/01/01 인 사람 중에서 급여가 2000000 원 이하인 사람의
-- 이름,주민번호,이메일,폰번호,급여를 검색 하시오
SELECT EMP_NAME "이름", EMP_NO "주민번호", PHONE "폰번호", SALARY "급여" FROM EMPLOYEE
...
WHERE SALARY BETWEEN 2000000 AND 3000000 AND EMP_NO LIKE '___4__-2%'
ORDER BY 2 DESC;
-- 문제6.
...
SELECT EMP_NAME "이름", FLOOR(FLOOR(SYSDATE - HIRE_DATE)/1000)*SALARY*0.1 "특별 보너스" 
FROM EMPLOYEE
