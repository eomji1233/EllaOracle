...
-- 1.1. 단일행 함수의 종류
-- 1.1.1 숫자 처리 함수
--         - ABS, MOD, TRUNK, FLOOR, ROUND, CEIL
-- 1.1.2 문자 처리 함수
...
--         g. CONCAT                : 문자열 합쳐줌
--         h. REPLACE               : 문자열 바꿔줌
--         사용법 : 함수명(값1, 값2, ...)으로 적고 값에는 리터럴이나 컬럼명을 넣어주면 됨
*/
-- 1.1.3 날짜 처리 함수
...
, (ADD_MONTHS(SYSDATE,18)-SYSDATE)*3 || '그릇' "짬밥수" FROM DUAL;
...
|| EXTRACT(MONTH FROM HIRE_DATE) || '월' "입사년월" FROM EMPLOYEE;
...
SELECT EMP_NAME "직원명"
, HIRE_DATE "입사일"
, FLOOR((SYSDATE-HIRE_DATE)) ||'일' "근무일수" FROM EMPLOYEE;
...
9           99999       해당자리의 숫자를 의미함. 값이 없을 경우 소수점이상은 공백, 소수점 이하는 0으로 표시.
0		    09999		해당자리의 숫자를 의미함. 값이 없을 경우 0으로 표시. 숫자의 길이를 고정적으로 표시할 경우.
$		    $9999		$ 통화로 표시
L		    L9999		Local 통화로 표시(한국의 경우 \)
XXXX		XXXX		16진수로 표시
FM         FM1234.56    포맷9로부터 치환된 공백(앞) 및 소수점이하 0을 제거
*/
...
-- DECODE 함수
-- 사용법 : DECODE(변하는 값, 조건값, 참일 때 값, 조건값, 참일 때 값, 모두 아닐 때 값)
-- 예시 : DECODE(변하는 값, 1, '빨강', 2, '노랑', '파랑')
-- CASE 함수
-- 사용법 : CASE (WHEN 변하는 값 = 조건값 THEN 참일 때 값) (WHEN 변하는 값 = 조건값 THEN 참일 때 값) (ELSE 모두 아닐 때 값) END
...
-- BONUS 컬럼의 값이 존재하면 그 행을 1로 카운팅, 보너스를 지급받은 사원이 없는 부서도 있음을 확인
SELECT NVL(DEPT_CODE,'부서없음'), COUNT(BONUS) FROM EMPLOYEE
--WHERE BONUS IS NOT NULL
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE ASC;
-- 실습3
-- EMPLOYEE 테이블에서 직급이 J1인 사람들을 제외하고 직급별 사원수 및 평균급여를 출력하세요.
SELECT JOB_CODE "직급", COUNT(JOB_CODE) "사원수", FLOOR(AVG(SALARY)) "평균급여" FROM EMPLOYEE
WHERE JOB_CODE != 'J1'
GROUP BY JOB_CODE;
-- 실습4
-- EMPLOYEE 테이블에서 직급이 J1인 사람들을 제외하고 입사년도별 인원수를 조회해서,
-- 입사년도 기준으로 오름차순으로 정렬하세요.
SELECT TO_CHAR(HIRE_DATE, 'YYYY') "입사년도", COUNT(*) "입사년도별 인원수" FROM EMPLOYEE
WHERE JOB_CODE != 'J1'
...
-- 성별 급여의 평균(정수처리), 급여의 합계, 인원수를 조회한 뒤 인원수로 내림차순을 정렬하시오.
SELECT DECODE(SUBSTR (EMP_NO,8,1), 1, '남', 2, '여', 3, '남', 4, '여') "성별" ,
FLOOR(AVG(SALARY)) "급여 평균", SUM(SALARY) "급여 합계", COUNT(SALARY) "인원수"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR (EMP_NO,8,1), 1, '남', 2, '여', 3, '남', 4, '여');

-- 지금까지 한 것
-- 숫자 처리 함수, 문자 처리 함수, 날짜 처리 함수
-- 그룹함수 결과이지만 GROUP BY를 쓰면 기준 별로 결과가 나옴
...
-- 실습7
-- 부서내 성별 인원수를 구하시오
SELECT DEPT_CODE, DECODE(SUBSTR (EMP_NO,8,1), 1, '남', 2, '여', 3, '남', 4, '여') "성별"
, COUNT(SALARY) "수"
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
-- SELECT문에서 조건을 걸 때에는 WHERE을 쓰지만
SELECT * FROM EMPLOYEE
WHERE SALARY >= 5000000;
-- GROUP BY한 결과값에 조건을 걸 때에는 HAVING을 쓴다.

-- 부서별 인원 수가 5명 미만인 레코드를 출력하세요
...
SELECT EMP_NAME "직원명", JOB_CODE "직급코드", '\' || SALARY "연봉" FROM EMPLOYEE;
