-- 5일차 JOIN
-- 함수 종합 실습 - 형변환 함수, 기타 함수

--10. 직원명, 직급코드, 연봉(원) 조회
--  단, 연봉은 ￦57,000,000 으로 표시되게 함
--     연봉은 보너스포인트가 적용된 1년치 급여임
SELECT EMP_NAME "직원명", JOB_CODE "직급코드", TO_CHAR(SALARY*12+SALARY*NVL(BONUS,0), 'L999,999,999') "연봉" FROM EMPLOYEE;
...
SELECT DECODE(ENT_YN, 'N', '재직', '퇴직') "상태", COUNT(*) FROM EMPLOYEE
GROUP BY ENT_YN;
...
--4. 보너스를 받는 직원들의 사원명, 보너스, 부서명, 근무지역명을 조회하시오.
...
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
INNER JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID; -- 21개임, 이오리 하동운 2개가 빠짐
SELECT COUNT (*) FROM EMPLOYEE; -- 23
-- 2개는 왜 빠졌나? - DEPT_CODE가 NULL인 데이터는 빠짐
-- -> 이것을 INNER JOIN이라고 함.

-- LEFT OUTER JOIN은 왼쪽 테이블이 가지고 있는 모든 데이터를 출력
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
LEFT OUTER JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID; -- 23개임, 이오리 하동운 2개가 안빠짐

-- RIGHT OUTER JOIN은 오른쪽 테이블이 가지고 있는 모든 데이터를 출력
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
RIGHT OUTER JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID; -- 24개임, 이오리 하동운 2개는 빠졌는데 안쓰인 해외영업, 마케팅, 국내영업부가 생김

-- FULL OUTER JOIN은 양쪽 테이블이 가지고 있는 모든 데이터를 출력
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
FULL OUTER JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID; -- 26개임, 전부 다 출력
