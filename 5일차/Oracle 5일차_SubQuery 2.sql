...
-- EXIST 안에있는 쿼리문이 RESULT SET이 나오는 쿼리문인지가 중요함
...
SELECT * FROM EMPLOYEE WHERE EXISTS (SELECT NULL FROM EMPLOYEE WHERE PHONE IS NULL);
...
SELECT 1 FROM EMPLOYEE WHERE MANAGER_ID = '213'; -- X
SELECT 1 FROM EMPLOYEE WHERE MANAGER_ID = '214'; -- O
SELECT * FROM EMPLOYEE WHERE EMP_ID IN ('200', '201', '204', '207', '211', '214');

SELECT * FROM EMPLOYEE E WHERE EXISTS(SELECT 1 FROM EMPLOYEE WHERE MANAGER_ID = E.EMP_ID);

-- EMPLOYEE가 가지고 있는 첫번째 행에서 MANAGER_ID 값이 200인 사람을 찾는데 
-- 같은 표에서 찾아버리면 모두 거짓이 됨
...
GROUP BY DEPT_CODE
HAVING DEPT_CODE = 'D5'; -- D5 부서의 월 평균 급여, 2번

SELECT DEPT_CODE, EMP_NAME
, (SELECT FLOOR(AVG(SALARY)) FROM EMPLOYEE
    GROUP BY DEPT_CODE
    HAVING DEPT_CODE = 'D5') "월 평균 급여"
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'; -- 메인쿼리에 SELECT 뒤에 월 평균 급여 서브쿼리 사용, 3번

-- D5를 구하는 부분을 모두 서브쿼리로 변경, 4번
SELECT DEPT_CODE, EMP_NAME, (SELECT FLOOR(AVG(SALARY)) FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING DEPT_CODE = (SELECT DEPT_CODE FROM EMPLOYEE WHERE EMP_NAME = '심봉선')) "월 평균 급여"
FROM EMPLOYEE
