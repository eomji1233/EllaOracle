...
FROM EMPLOYEE;
-- 주의 : VIEW를 만들 때 에는 권한이 필요함, 권한명 : CREATE VIEW
-- System 계정으로 권한 부여
...
AS SELECT EMP_ID, EMP_NAME, JOB_NAME, DEPT_TITLE, LOCAL_NAME FROM EMPLOYEE
...
-- VIEW와 원본은 링크가 되어 있어서 VIEW 수정 시 원본도 수정됨
-- 확인했으면 ROLLBACK!
ROLLBACK;
...
FROM EMPLOYEE; -- name is already used by an existing object
...
FROM EMPLOYEE;
-- 1.5.2 FORCE/NOFORCE
-- 기존 테이블이 존재하지 않더라도 VIEW를 생성하는 옵션
...
SELECT * FROM DBA_TABLES; -- System 계정으로 실행
...
-- 2.2.5 NOCYCLE : 시퀀스 값이 최대값까지 증가를 완료하면 CYCLE은 START WITH로 다시 시작함
-- 2.2.6 NOCACHE : 메모리 상에서 시퀀스값을 관리, 기본값은 20, NOCACHE를 안하면 갑자기 시퀀스 값이 증가하게 됨
...
SELECT * FROM USER_TCL ORDER BY USER_NO;
...
INSERT INTO KH_MEMBER
VALUES (SEQ_MEMBER_ID.NEXTVAL, '청길동', 30, SEQ_JOIN_COM.NEXTVAL);
INSERT INTO KH_MEMBER
VALUES (SEQ_MEMBER_ID.NEXTVAL, '외길동', 40, SEQ_JOIN_COM.NEXTVAL);
INSERT INTO KH_MEMBER
VALUES (SEQ_MEMBER_ID.NEXTVAL, '고길동', 50, SEQ_JOIN_COM.NEXTVAL);

DELETE FROM KH_MEMBER;
DROP SEQUENCE SEQ_MEMBER_ID;
DROP SEQUENCE SEQ_JOIN_COM;
