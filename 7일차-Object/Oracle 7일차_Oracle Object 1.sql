...
-- 프로시저(PRODEDUAL), 함수(FUNCTION), 트리거(TRIGGER), 동의어(SYSNONYM), 커서(CUROSR), 패키지(PACKAGE), ..
...
-- 권한부여 및 회수는 관리자 세션(RED)에서 사용 가능
-- 관리자 계정
...
-- - KEY와 VALUE 형태로 생성이 되며 KEY에는 인덱스로 만들 컬럼값, VALUE에는 행이 저장된
-- 주소값이 저장됨
-- - 장점 : 검색속도가 빨라지고 시스템에 걸리는 부하를 줄여서 시스템 전체 성능을 향상시킬 수 있음
-- - 단점 : 인덱스를 위한 추가 저장 공간이 필요하고 인덱스를 생성하는데 시간이 걸림
-- 데이터의 변경 작업이 자주 일어나는 테이블에 INDEX 생성 시 오히려 성능저하가 발생할 수 있음
-- - 어떤 컬럼에 인덱스를 만들면 좋을까?
-- 데이터 값이 중복된 것이 없는 고유한 데이터 값을 가지는 컬럼에 만드는 것이 가장 좋음
...
WHERE EMP_ID LIKE '2%';
SET TIMING OFF;
-- F10으로 실행하여 오라클 PLAN 확인 가능, 튜닝 시 사용됨
