-- 8일차 PL/SQL
-- Oracle Procedural Language Extension to SQL의 약자
...
BEGIN
    ...
    WHERE EMP_ID = '&사번';
    SALARY := SALARY / 10000;
    IF (SALARY BETWEEN 0 AND 99) THEN SLEVEL := 'F';
    ...
    DBMS_OUTPUT.PUT_LINE('급여 : ' || SALARY*10000 || '원');
...
-- EXIT 절이 없어도 조건절 반복문 중지 조건을 기술할 수 있음.
...
BEGIN
    ...
    LOOP
        ...
        ' 사원명 : ' || EINFO.EMP_NAME || ' 이메일 : ' || EINFO.EMAIL);
    END LOOP;
END;
/
...
-- EXCEPTION WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE('노 데이터!');
-- NO_DATA_FOUND
