...
UPDATE EMPLOYEE SET SALARY = '9000000' WHERE EMP_NAME = '선동일';

CREATE OR REPLACE TRIGGER TRG_SAL_UPDATE
AFTER
UPDATE ON EMPLOYEE
FOR EACH ROW
BEGIN
    IF(:OLD.SALARY != :NEW.SALARY) -- EMPLOYEE 테이블에 무조건 UPDATE가 되면 실행되는 것을 방지
    THEN
        DBMS_OUTPUT.PUT_LINE('급여 정보가 변경되었습니다.');
        DBMS_OUTPUT.PUT_LINE('변경 전 : ' || :OLD.SALARY); -- 변경 전의 레코드(OLD)
        DBMS_OUTPUT.PUT_LINE('변경 전 : ' || :NEW.SALARY); -- 변경 후의 레코드(NEW)
    END IF;
END;
/
COMMIT;
...
CREATE TABLE PRODUCT_IO (
    ...
    PCODE NUMBER, -- CONSTRAINT FK_PRODUCT_IO REFERENCES PRODUCT(PCODE) -- 컬럼레벨
    PDATE DATE,
    AMOUNT NUMBER,
    STATUS VARCHAR2(10) CHECK(STATUS IN('입고', '출고')),
    CONSTRAINT FK_PRODUCT_IO FOREIGN KEY(PCODE) REFERENCES PRODUCT(PCODE) -- 테이블레벨
);
