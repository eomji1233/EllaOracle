...
-- 나중에 많은 컬럼 추가할 때 편하게 할라고 E.*로 별칭 추가
SELECT ROWNUM, E.* 
FROM (SELECT SALARY FROM EMPLOYEE WHERE SALARY IS NOT NULL ORDER BY 1 DESC) E
WHERE ROWNUM < 11;
-- 그냥 추가한 버전
SELECT ROWNUM, SALARY
FROM (SELECT SALARY FROM EMPLOYEE WHERE SALARY IS NOT NULL ORDER BY 1 DESC)
WHERE ROWNUM < 11;
...
SELECT ROWNUM, E.*
FROM (SELECT * FROM EMPLOYEE WHERE DEPT_CODE = 'D5' ORDER BY SALARY DESC) E
WHERE ROWNUM < 4;
...
-- 2. WITH
-- 서브쿼리에 이름을 붙여주고 인라인뷰로 사용 시 
-- 서크쿼리의 이름을 FROM절에 기술할 수 있음
...
WHERE ROWNUM < 4 AND DEPT_CODE = 'D5';
...
GROUP BY DEPT_CODE, DEPT_TITLE
ORDER BY 3 DESC)
SELECT * FROM TOP3_AVG
WHERE ROWNUM < 4;

-- 3. 계층형 쿼리
-- JOIN을 통해 수평적으로 기준 컬럼을 연결시킨 것과는 달리 
-- 기준 컬럼을 가지고 수직적인 관계를 만듬
...
-- 4. LEVEL : 의사컬럼(PCEUDO COLUMN), 계층정보를 나타내는 가상컬럼이며
...
SELECT
    EMP_NAME "이름"
    , HIRE_DATE "입사일"
    , RANK() OVER(ORDER BY HIRE_DATE) "순위"
FROM EMPLOYEE
...
-- RANK는 19,19 있으면 다음이 21위였는데 얘는 20위로 출력
...
SELECT * FROM
(SELECT EMP_NAME "이름", SALARY "급여", DENSE_RANK() OVER(ORDER BY SALARY DESC) "순위"
FROM EMPLOYEE)
...
FROM EMPLOYEE
WHERE ROWNUM < 11;
-- ROWNUM이랑 순위랑 다를 수 있기 때문에 RANK를 써주는게 좋음
