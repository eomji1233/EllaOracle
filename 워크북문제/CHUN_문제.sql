...
SELECT PROFESSOR_NO FROM TB_PROFESSOR WHERE CLASS_NO IS NOT NULL;

SELECT PROFESSOR_NAME
FROM TB_PROFESSOR 
LEFT OUTER JOIN TB_CLASS_PROFESSOR USING (PROFESSOR_NO)
ORDER BY 1 ASC
WHERE PROFESSOR_NO IS NULL;

-- 10. 02 학번 전주 거주자들의 모임을 만들려고 한다. 휴학한 사람들은 제외한 재학중인
-- 학생들의 학번, 이름, 주민번호를 출력하는 구문을 작성하시오.
SELECT STUDENT_NO, STUDENT_NAME, STUDENT_SSN, STUDENT_ADDRESS, ENTRANCE_DATE FROM TB_STUDENT
...
-- 한 자리까지만 표시한다.
SELECT STUDENT_NO, STUDENT_NAME, (SELECT ROUND(AVG(POINT),1) FROM TB_GRADE WHERE STUDENT_NO = S.STUDENT_NO) "평점" 
FROM TB_STUDENT S
...
WHERE (SELECT STUDENT_NAME FROM TB_STUDENT WHERE STUDENT_NAME = S.STUDENT_NAME);

SELECT * FROM TB_STUDENT S
WHERE EXISTS (SELECT 1 FROM TB_STUDENT WHERE STUDENT_NAME = S.STUDENT_NAME);
...
SELECT STUDENT_NAME, NVL(PROFESSOR_NAME, '지도교수 미지정') "지도교수", CLASS_NAME FROM TB_STUDENT
LEFT OUTER JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
LEFT OUTER JOIN TB_CLASS USING(DEPARTMENT_NO)
LEFT OUTER JOIN TB_CLASS_PROFESSOR ON COACH_PROFESSOR_NO = PROFESSOR_NO
LEFT OUTER JOIN TB_PROFESSOR USING (PROFESSOR_NO)
